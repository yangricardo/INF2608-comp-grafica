...
\newcommand{\pbrtsec}[1]{\href{\pbrtsectionurl{#1}}{PBRT 4e, Se\c{c}\~ao~#1}~\citep{pharr2023pbrt}}
\newcommand{\pbrtsecs}[1]{\href{\pbrtcontentsurl}{PBRT 4e, Se\c{c}\~oes~#1}~\citep{pharr2023pbrt}}
...
Este artigo apresenta uma análise técnico-científica da implementação de ray tracing disponível no repositório do Projeto 1~\citep{inf2608projectrepo}. O objeto principal da análise é o código em \path{src/ray_tracing_2/}, confrontado com os tópicos dos materiais da disciplina~\citep{inf2608slides04,inf2608slides05,inf2608slides06} e com referências selecionadas do PBRT 4e, sobretudo \pbrtsecs{1.2, 3.5, 3.7, 5.2, 7.3, 8.1, 8.5, 9.3, 9.5, 11.2 e 12.4}.
...
Como fonte externa, foi usado apenas o conteúdo de PBRT 4e permitido pelo enunciado, com ênfase em câmera projetiva, teoria de amostragem, reflexão e transmissão especular, transmitância, luzes de área, caixas envolventes e BVHs. A edição consultada foi a quarta edição online de PBRT~\citep{pharr2023pbrt}.
...
\subsection{Quadro de aderência do núcleo geométrico e fotométrico}
\label{sec:slide4-quadro}

\begin{table*}[!ht]
\caption{Quadro de aderência entre o núcleo geométrico-fotométrico de referência e a implementação atual.}
...
O quadro mostra que o núcleo conceitual discutido no material introdutório~\citep{inf2608slides04} está efetivamente materializado no repositório, mas com duas qualificações importantes: a luz pontual segue a convenção simplificada do enunciado, e o teste de sombra já aparece no código em forma mais geral do que a apresentada no bloco introdutório dos slides.
...
\subsection{Quadro de aderência das extensões ópticas e amostrais}
\label{sec:slide5-quadro}

\begin{table*}[!ht]
\caption{Quadro de aderência entre as extensões ópticas e amostrais de referência e a implementação atual.}
...
O quadro mostra que o conjunto de extensões ópticas e amostrais discutido nos slides~\citep{inf2608slides05} é amplamente coberto pelo repositório atual, mas quase sempre em versões computacionalmente controladas: a recursão é truncada por profundidade, a luz de área é estimada por soma discreta com padrão configurável (\texttt{regular}, \texttt{uniform}, \texttt{stratified}), e o dielétrico permanece no regime homogêneo RGB com Schlick e Beer-Lambert, sem migrar para uma formulação espectral ou microfacet completa.
...
\begin{table*}[!ht]
\caption{Síntese da avaliação final do que a implementação entrega efetivamente.}
...
Núcleo geométrico e fotométrico & Entregue & câmera pinhole, raio paramétrico, \texttt{Hit}, interseções analíticas para esfera, plano e caixa, \texttt{closest hit}, Phong local, sombra direta e luz ambiente \\
...
Extensões ópticas e amostrais & Entregue com simplificações declaradas & instanciação por transformações afins, luz de área com penumbra, reflexão com Fresnel-Schlick, refração com Snell, reflexão interna total e Beer-Lambert por canal RGB \\
...
Cobertura integral dos materiais da disciplina e do PBRT & Não integral & faltam câmera além de pinhole, coerência radiométrica uniforme entre todas as luzes, materiais fisicamente baseados completos e estruturas globais de aceleração \\
\end{tblr}
\end{table*}
