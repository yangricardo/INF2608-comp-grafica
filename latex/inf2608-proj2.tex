...
\end{figure}
\figcmd{python -m path_tracing.scripts.proj2_req5_rr --mode mis --use-rr true --spp 16 --depth 8 --seed 42}
...
A fonte é um \textbf{octaedro} emissivo (8 faces triangulares) suspenso no teto --- um
poliedro nítido, e não um quad plano --- com amostragem uniforme por área. O mesmo octaedro
é geometria visível e luz de NEE.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.92\linewidth]{../out/proj2/req6/proj2_req6_mis_no_rr_20260606_205410/render.png}
\caption{Luz poliédrica --- octaedro emissivo (8 faces) com amostragem uniforme por área, MIS (512$\times$512, SPP=16).}
\end{figure}
\figcmd{python -m path_tracing.scripts.proj2_req6_mesh_lights --spp 16 --depth 8 --mode mis}

\noindent\textbf{Variações de parâmetros} (malha poliédrica com MIS e com Roleta Russa):
\begin{itemize}\small\setlength{\itemsep}{1pt}
\item \texttt{-{}-mode bsdf\_only -{}-spp 16} vs.\ \texttt{-{}-mode mis -{}-spp 16} --- a luz menor (octaedro) realça o ganho do NEE/MIS.
\item \texttt{-{}-mode mis -{}-use-rr true -{}-spp 32} --- combina amostragem uniforme por área e RR.
\end{itemize}
...
\caption{Esfera de vidro incolor (IOR=1.5), MIS sem Roleta Russa (800$\times$600, SPP=64, seed=42). Evidencia refração, Fresnel e reflexão interna total.}
\end{figure}
\begin{figure}[!ht]
\centering
\includegraphics[width=0.92\linewidth]{../out/proj2/req7/proj2_req7_water_mis_no_rr_20260606_212027/render.png}
\caption{Esfera de água (IOR=1.33) com absorção de Beer-Lambert, MIS sem Roleta Russa (512$\times$512, SPP=256). Coloração azul-esverdeada por atenuação seletiva do vermelho.}
\end{figure}
\figcmd{python -m path_tracing.scripts.proj2_req7_dielectric --material glass --spp 64 --depth 8 --mode mis --seed 42}
...
Para comparar os estimadores a partir de \textbf{uma única cena}, a \emph{showcase}
(\path{src/path_tracing/scenes/cornell_showcase.py}) reúne, numa Cornell Box: duas caixas
difusas rotacionadas, uma esfera de vidro incolor, uma esfera de vidro \textbf{âmbar}
(absorção de Beer-Lambert), uma esfera difusa azul (\emph{color bleeding}) e uma
\textbf{luz poliédrica} --- um octaedro emissivo (8 faces) que serve de geometria visível e
de luz para NEE. Assim, GI difusa, dielétrico (incolor e absorvente) e luz de malha são
exercitados juntos. O script renderiza os quatro modos e grava cópias estáveis em
\path{out/proj2/showcase/} (incluídas automaticamente abaixo quando existirem).

\figcmd{python -m path_tracing.scripts.proj2_showcase --all --spp 64 --depth 8 --seed 42}

\noindent Equivalente, modo a modo:
\begin{itemize}\small\setlength{\itemsep}{1pt}
\item \texttt{-{}-mode bsdf\_only -{}-spp 64} \quad \texttt{-{}-mode nee\_only -{}-spp 64} \quad \texttt{-{}-mode mis -{}-spp 64}
\item \texttt{-{}-mode mis -{}-use-rr true -{}-spp 64} (Roleta Russa)
\end{itemize}

\figmaybe{../out/proj2/showcase/proj2_showcase_bsdf_only_no_rr.png}{Showcase --- \texttt{bsdf\_only} (alta variância; luz pequena raramente amostrada pela BSDF).}
\figmaybe{../out/proj2/showcase/proj2_showcase_nee_only_no_rr.png}{Showcase --- \texttt{nee\_only} (amostragem direta da luz poliédrica).}
\figmaybe{../out/proj2/showcase/proj2_showcase_mis_no_rr.png}{Showcase --- \texttt{mis} (combina BSDF e NEE; menor ruído).}
\figmaybe{../out/proj2/showcase/proj2_showcase_mis_rr.png}{Showcase --- \texttt{mis} + Roleta Russa (mesma média, menor custo).}

% =====================================================================
\subsection{Cena \emph{showcase} com luzes de parede hexagonais}
\label{sec:showcase-wall}

A cena \emph{wall lights} (\path{src/path_tracing/scenes/cornell_wall_lights.py}) estende a
Cornell Box com \textbf{três} fontes de área distintas: um painel retangular emissivo no teto
(branco) e dois hexágonos emissivos nas paredes laterais (verde à esquerda, vermelho à
direita). O ambiente inclui ainda duas esferas dielétricas --- vidro incolor (IOR 1.5) e
âmbar com absorção de Beer-Lambert --- uma esfera difusa azul e duas caixas rotacionadas.
Essa composição exercita simultaneamente GI difusa, refração, absorção cromática e amostragem
de múltiplas fontes de área com geometria não-planar.
...
\figcmd{python -m path_tracing.scripts.proj2_showcase_wall_lights --all --spp 64 --depth 8 --seed 42}

\noindent Equivalente, modo a modo:
\begin{itemize}\small\setlength{\itemsep}{1pt}
\item \texttt{-{}-mode bsdf\_only -{}-spp 64}
\item \texttt{-{}-mode nee\_only -{}-spp 64}
\item \texttt{-{}-mode mis -{}-spp 64}
\item \texttt{-{}-mode mis -{}-use-rr true -{}-spp 64} (Roleta Russa)
\end{itemize}

\figmaybe{../out/proj2/showcase_wall_lights/proj2_showcase_wall_lights_bsdf_only_no_rr.png}{Wall lights --- \texttt{bsdf\_only}: alta variância pois as fontes hexagonais laterais são raramente amostradas por BSDF.}
\figmaybe{../out/proj2/showcase_wall_lights/proj2_showcase_wall_lights_nee_only_no_rr.png}{Wall lights --- \texttt{nee\_only}: amostragem direta das três fontes (teto + paredes); iluminação direta nítida.}
\figmaybe{../out/proj2/showcase_wall_lights/proj2_showcase_wall_lights_mis_no_rr.png}{Wall lights --- \texttt{mis}: combina BSDF e NEE com heurística de potência; menor variância em superfícies difusas e especulares.}
\figmaybe{../out/proj2/showcase_wall_lights/proj2_showcase_wall_lights_mis_rr.png}{Wall lights --- \texttt{mis} + Roleta Russa: mesma média com terminação probabilística de caminhos profundos.}
