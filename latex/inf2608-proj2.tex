...
Susp.-dir. & (4.10, 2.60, 2.20) & 0.75 & 1{,}33 & água azul \\
...
A cena combinada integra todos os recursos implementados: 3 fontes de luz (1 retangular no teto + 2 hexagonais nas paredes),
4 esferas dielétricas (2 no piso com IOR=1{,}5, 1 suspensa com IOR=1{,}33) substituídas por 1 esfera espelhada (reflexão pura),
2 caixas Lambertianas rotacionadas e 1 esfera azul (color bleeding). A esfera espelhada na posição suspensa-dir.\ reflete as 3 fontes de luz coloridas, gerando
padrões de reflexão especular que contrastam com a refração e absorção das esferas dielétricas.

\figmaybe{../out/proj2/showcase_combined/proj2_showcase_combined_bsdf_only_no_rr.png}{Cena combinada com esfera espelhada, \texttt{bsdf\_only} (512$\times$512, SPP=4, estratificado, depth=4): alta variância; reflexões especulares da esfera espelhada amostradas raramente via BSDF puro; esferas dielétricas com regiões escuras por \emph{depth} esgotado.}
\figmaybe{../out/proj2/showcase_combined/proj2_showcase_combined_nee_only_no_rr.png}{Cena combinada com esfera espelhada, \texttt{nee\_only} (512$\times$512, SPP=4, estratificado, depth=4): iluminação direta das três fontes nítida nas superfícies difusas; esfera espelhada e dielétricas especulares ainda ruidosas pois NEE é pulado em bounces dielétricos (\texttt{is\_specular}).}

\figmaybe{../out/proj2/showcase_combined/proj2_showcase_combined_mis_no_rr.png}{Cena combinada com esfera espelhada, \texttt{mis} (512$\times$512, SPP=4, estratificado, depth=4): combina BSDF e NEE com heurística de potência ($\beta=2$); reflexão especular da esfera espelhada bem definida; menor variância nas superfícies difusas; regiões TIR visíveis nas esferas dielétricas.}

\figmaybe{../out/proj2/showcase_combined/proj2_showcase_combined_mis_rr.png}{Cena combinada com esfera espelhada, \texttt{mis} + Roleta Russa (512$\times$512, SPP=4, estratificado, depth=4): terminação probabilística de caminhos de baixo \emph{throughput}; esfera espelhada reflete múltiplas fontes com redução de tempo de render mantendo estimador não-viesado.}
{\small\figcmd{python -m path_tracing.scripts.proj2_showcase_combined --all --spp 20 --depth 8}}
