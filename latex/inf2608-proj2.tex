...
$w_\text{bsdf} = p_\text{bsdf}^2 / \bigl(p_\text{bsdf}^2 + \sum_i p_{L_i}^2\bigr)$,
mais preciso que \texttt{power\_heuristic}$(1,\,p_\text{bsdf},\,1,\,\sum_i p_{L_i})$,
pois $\bigl(\sum_i p_{L_i}\bigr)^2 \neq \sum_i p_{L_i}^2$ para $N>1$
...
\paragraph{Espelho perfeito (reflexão especular pura).}
Superfícies espelhadas implementam \texttt{MirrorBSDF} — reflexão delta (100\%) sem refração
nem absorção. Contrasta com \texttt{DielectricBSDF} (vidro) que permite tanto reflexão
quanto transmissão. O espelho é o caso limite: Fresnel$= 1$, nenhuma transmissão.

As 4 variações a seguir ilustram como os modos de amostragem (\textit{bsdf\_only}, \textit{nee\_only}, \textit{mis}, \textit{mis}+RR)
afetam a qualidade e convergência da renderização de uma esfera espelhada:

\figmaybe{../out/proj2/req7_mirror/proj2_req7_mirror_bsdf_only_no_rr.png}{Espelho, \texttt{bsdf\_only} (512×512, SPP=20, sem RR): alta variância; reflexões da luz direta são raramente amostradas via BSDF puro, resultando em ruído dominante nas regiões não-especulares.}

\figmaybe{../out/proj2/req7_mirror/proj2_req7_mirror_nee_only_no_rr.png}{Espelho, \texttt{nee\_only} (512×512, SPP=20, sem RR): iluminação direta nítida via amostragem da área de luz (NEE); as reflexões da esfera mostram apenas componentes de luz direta amostrada; regiões bloqueadas são completamente pretas.}

\figmaybe{../out/proj2/req7_mirror/proj2_req7_mirror_mis_no_rr.png}{Espelho, \texttt{mis} (512×512, SPP=20, sem RR): MIS combina BSDF + NEE com heurística de potência; menor variância que \textit{bsdf\_only}, detalhes visuais mais ricos que \textit{nee\_only}; reflexões especulares bem definidas.}

\figmaybe{../out/proj2/req7_mirror/proj2_req7_mirror_mis_rr.png}{Espelho, \texttt{mis} + Roleta Russa (512×512, SPP=20, com RR): mesma estratégia que \textit{mis}, mas com terminação probabilística de caminhos profundos; renderiza ~10--30\% mais rápido com erro visual similar.}

{\small\figcmd{python -m path_tracing.scripts.proj2_req7_mirror --all --spp 20 --depth 8}}

\noindent\textbf{Comparação de modos de amostragem (esfera espelhada):}
\begin{center}\tiny
\begin{tabular}{l@{\,}l@{\,}l}
\textbf{Modo} & \textbf{Parâmetros} & \textbf{Característica visual} \\
\hline
\texttt{bsdf\_only} & SPP=20, sem NEE, sem RR & Alta variância; reflexões ruidosas \\
\texttt{nee\_only} & SPP=20, NEE direto, sem RR & Iluminação direta nítida; sem indiretos \\
\texttt{mis} & SPP=20, BSDF+NEE, sem RR & Balanceado; menor variância \\
\texttt{mis}+RR & SPP=20, BSDF+NEE, com RR & Mesma média; ~10--30\% mais rápido \\
\end{tabular}
\end{center}

\noindent\textbf{Análise das variações visuais:}

\noindent\textit{Modo \texttt{bsdf\_only}} (esquerda superior) amostra apenas a distribuição especular da BSDF pura, sem luz direta estruturada. Cada caminho de raio busca reflexões via Monte Carlo ingênuo, resultando em alta variância. As regiões do piso que refletem a esfera mostram ruído branco dominante, pois a probabilidade de um raio amostrado pela BSDF acertar a fonte de luz é baixa. Convergência lenta (∼ $O(1/\sqrt{\text{SPP}})$).

\noindent\textit{Modo \texttt{nee\_only}} (direita superior) amostras a iluminação direta explicitamente via NEE: para cada ponto, submete um raio de sombra à fonte de luz. O resultado é nítido e sem ruído nas regiões iluminadas, pois NEE reduz variância a zero para luz direta. Porém, reflexões indiretas da esfera (aquelas que não veem a fonte diretamente após um bounce) são completamente pretas, limitando riqueza visual. A esfera aparece bem definida, mas o piso reflete um padrão de iluminação quase binário (iluminado ou sombra).

\noindent\textit{Modo \texttt{mis}} (esquerda inferior) combina BSDF e NEE com múltipla importância: estima iluminação direta tanto via BSDF quanto via NEE, aplicando a heurística de potência ($\beta=2$) para pesar as duas estimativas. Resultado: convergência mais rápida que \texttt{bsdf\_only} (ambas as estratégias reduzem variância) e mais riqueza visual que \texttt{nee\_only} (reflexões indiretas são amostradas). A esfera e o piso mostram transições suaves entre luz e sombra, com reflexões bem definidas.

\noindent\textit{Modo \texttt{mis}+RR} (direita inferior) aplica a mesma estratégia MIS, mas termina caminhos profundos probabilisticamente: caminhos com \emph{throughput} baixo são interrompidos com probabilidade crescente, reduzindo custo computacional. A estimativa permanece não-viesada (mantém esperança igual), resultando em render ~10--30\% mais rápido com erro visual praticamente imperceptível em SPP=20. Visualmente idêntico a \texttt{mis}, mas com tempo de processamento reduzido.
...
\paragraph{Cena combinada com esfera espelhada.}
A composição final integra 3 fontes de luz coloridas (retangular no teto + duas hexagonais nas
paredes verde e vermelha), 3 esferas dielétricas (duas no piso com IOR=1{,}5; uma suspensa com
IOR=1{,}33) e \textbf{uma esfera espelhada} (\texttt{MirrorBSDF}, reflexão pura) na posição
suspensa-direita, além de 2 caixas Lambertianas rotacionadas e 1 esfera difusa azul
(\emph{color bleeding}). A esfera espelhada reflete as 3 fontes coloridas e o ambiente,
produzindo reflexão especular nítida que contrasta com a refração/absorção das dielétricas.
As oito figuras a seguir comparam os quatro modos de amostragem, cada um em SPP=4 (rápido,
ruidoso) e SPP=32 (convergido), evidenciando a redução de variância $\propto 1/\sqrt{\text{SPP}}$.
...
\noindent\textbf{Análise: Variações de modo em cena complexa com esfera espelhada.}

A cena combinada integra 3 fontes de luz coloridas (teto branco, paredes verde e vermelha),
3 esferas dielétricas com refração/absorção, 1 esfera espelhada com reflexão pura, e 2 caixas difusas.
Essa complexidade amplifica as diferenças entre modos de amostragem:

\noindent\textit{Modo \texttt{bsdf\_only}} domina em ruído branco puro. A amostragem aleatória
da BSDF falha em capturar iluminação estruturada; as reflexões especulares (esfera espelhada
e bordas dielétricas) aparecem como ``pimenta e sal'' típico de Monte Carlo ingênuo.
Tempo: 19{,}6 min (mais rápido, mas inútil sem convergência). Convergência: $O(1/\sqrt{\text{SPP}})$.

\noindent\textit{Modo \texttt{nee\_only}} inverte o problema: iluminação direta é nítida,
mas reflexões indiretas desaparecem. As 3 fontes brilham perfeitamente mapeadas nas superfícies;
porém, a esfera espelhada reflete apenas essas fontes diretas (sem reverberação).
Não há color bleeding --- efeito fisicamente correto mas visualmente vazio.
Tempo: 47{,}3 min (2.4× mais lento que bsdf\_only).

\noindent\textit{Modo \texttt{mis}} atinge equilíbrio: NEE elimina ruído de iluminação direta,
enquanto BSDF captura reflexões indiretas. A esfera espelhada reflete cores do ambiente
(efeito ``color bleeding'' visível nas bordas), as dielétricas mostram cáusticas ricas,
e transições luz-sombra são suaves e naturais. Tempo: 50{,}7 min. Convergência mais rápida que bsdf\_only,
riqueza visual superior a nee\_only.

\noindent\textit{Modo \texttt{mis}+RR} aplica Roleta Russa: caminhos com low throughput
terminam cedo com probabilidade calibrada. Resultado: 28{,}6\% mais rápido que MIS sem RR (36{,}2 min),
visualmente imperceptível em SPP=32. Trade-off clássico de Monte Carlo:
reduza custo computacional mantendo estimador não-viesado.

\noindent\textbf{Comparação: Modos em cena combinada com esfera espelhada.}
\begin{center}\tiny
\begin{tabular}{l@{\,}l@{\,}l@{\,}l}
\textbf{Modo} & \textbf{Parâmetros} & \textbf{Variância} & \textbf{Tempo (min)} \\
\hline
\texttt{bsdf\_only} & SPP=32, sem NEE, sem RR & Muito alta (sal e pimenta) & 19{,}6 \\
\texttt{nee\_only} & SPP=32, NEE direto, sem RR & Baixa (direto), sombras pretas & 47{,}3 \\
\texttt{mis} & SPP=32, BSDF+NEE, sem RR & Moderada (balanceado) & 50{,}7 \\
\texttt{mis}+RR & SPP=32, BSDF+NEE, com RR & Idêntica a MIS & 36{,}2 (−28{,}6\%) \\
\end{tabular}
\end{center}
