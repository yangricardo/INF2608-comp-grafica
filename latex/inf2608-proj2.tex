...
BSDF dielétrica refrativa (Seção~\ref{sec:dielectric}); como extra, a reflexão especular pura
(espelho) é tratada junto às evidências (Seção~\ref{sec:mirror}). As evidências visuais estão
...
\emph{não} participam de MIS: \texttt{pending\_mis\_weight} é resetado a $1{,}0$ e NEE é
suprimido no vértice especular. \textbf{Esta é implementação correta}, não um defeito.
...
\subsection{Espelho: reflexão especular pura (extra)}
\label{sec:mirror}
