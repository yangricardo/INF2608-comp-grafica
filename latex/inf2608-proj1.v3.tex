...
%% Versão v3 em LaTeX com corpo teórico reforçado e rastreabilidade consolidada.
...
\begin{abstract-pt}
Este artigo apresenta uma análise técnico-científica da implementação de ray tracing desenvolvida no Projeto 1 da disciplina de INF2608. O objetivo é verificar, a partir do código efetivamente executável, quais componentes geométricos, fotométricos e amostrais foram materializados, como eles se relacionam com os slides da disciplina e em que medida o resultado se aproxima de referências selecionadas de PBRT 4e. A versão atual recupera a estrutura teórica mais próxima da primeira redação do relatório, reforça as explicações matemáticas sobre formação da imagem, interseções, iluminação local, amostragem, reflexão, refração e geometria triangular, e desloca a rastreabilidade detalhada para quadros finais de referência e evidência com notas sobre páginas dos slides e linhas de código.
\end{abstract-pt}

\begin{abstract-en}
This paper presents a technical-scientific analysis of the ray tracing implementation developed for the INF2608 course project. Its goal is to verify, from the executable code itself, which geometric, photometric, and sampling components were materialized, how they relate to the course slides, and to what extent the result aligns with selected PBRT 4e references. This version restores a theory-oriented structure, strengthens the mathematical discussion of image formation, intersections, local illumination, sampling, reflection, refraction, and triangle geometry, and moves detailed traceability to final reference-and-evidence tables with notes pointing to slide pages and source-code lines.
\end{abstract-en}
...
Este artigo analisa a implementação de ray tracing disponível no repositório do Projeto 1~\citep{inf2608projectrepo}. O objeto principal da análise é o código em \path{src/ray_tracing_2/}, confrontado com os tópicos dos slides da disciplina, de autoria de Waldemar Celes~\citep{inf2608slides04,inf2608slides05,inf2608slides06}, e com referências selecionadas de \pbrtbook~\citep{pharr2023pbrt}. O objetivo não é recontar genericamente o que um ray tracer deveria fazer, mas examinar o que esta base entrega de forma observável, quais decisões algorítmicas sustentam cada efeito visual e onde o projeto simplifica o conteúdo teórico.

A organização desta versão retoma a espinha teórica da versão longa do relatório. Em vez de separar o corpo principal por requisito do enunciado, o texto passa a apresentar o pipeline do traçador, o núcleo geométrico e fotométrico, as extensões ópticas e amostrais, a geometria triangular e as estruturas de aceleração. As imagens principais ficam reunidas em uma seção específica de evidências visuais, na qual cada figura é acompanhada por uma leitura do que ela comprova e do que ela não comprova.

A principal mudança editorial em relação à redação anterior é o deslocamento da rastreabilidade detalhada para os anexos finais. Em vez de interromper a argumentação com referências locais a cada subseção, o Anexo A reúne quadros consolidados de \emph{conceitos de referência}, \emph{evidência principal} e \emph{nota}, em que as notas sintetizam as páginas dos slides, as seções mais próximas em PBRT e as linhas de código mais relevantes. O Anexo B mantém o diagrama de classes como apoio estrutural à leitura da arquitetura.
...
O pipeline do traçador pode ser lido como uma composição de quatro etapas. Primeiro, o filme define a amostragem do pixel; depois, a câmera converte a amostra em um raio primário; em seguida, a cena resolve a primeira interseção visível; por fim, o material associado ao \emph{hit} calcula a contribuição local e, quando necessário, dispara raios secundários. Em forma resumida, a cor do pixel pode ser entendida como
...
Há ainda uma nuance importante de modelagem. A formulação didática clássica para fonte pontual costuma incluir decaimento geométrico por \(1/r^2\); nesta base, \texttt{PointLight} preserva a convenção simplificada do enunciado e trata \texttt{power} como radiância incidente constante. Não se trata de erro, mas de simplificação deliberada, que será retomada na seção de limites.
...
As figuras desta seção usam as cenas dedicadas da versão orientada a requisitos e algumas extensões selecionadas. O objetivo não é substituir a análise teórica anterior, mas oferecer evidência visual organizada do que ela descreve. Em cada caso, a pergunta orientadora é a mesma: o que exatamente a figura mostra e que hipótese técnica ela ajuda a sustentar.
...
\caption{Cena com múltiplas luzes pontuais e variação do brilho especular em materiais com \texttt{shininess} distinto (800x600, spp=1).}
\label{fig:req2-v3}
\end{figure}
...
Na Figura~\ref{fig:req4-v3}, a geometria foi escolhida para que arestas finas e contrastes fortes deixem o aliasing visível. A imagem inserida no relatório corresponde ao baseline \texttt{center}; sua função é servir como referência para comparações com \texttt{jittered} e \texttt{stratified}. O ponto importante não é provar convergência estatística, mas deixar claro que a base já separa câmera, geometria e amostragem do pixel.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.92\linewidth]{../outputs/proj1_req4_sampling_20260502_122009/render.png}
\caption{Cena-base para comparação entre \texttt{center}, \texttt{jittered} e \texttt{stratified} no domínio do pixel (800x600, spp=1).}
\label{fig:req4-v3}
\end{figure}
...
A parte de triângulos e aceleração cobre apenas uma fração do arco teórico da disciplina. Há BVH estática por \texttt{TriangleMesh}, com AABB e travessia recursiva simplificada, mas não há grade regular, SAH, BVH linearizada nem estrutura global de aceleração. Em termos de custo, isso significa que a interseção no topo da cena continua linear no número de objetos, e o ganho de desempenho aparece apenas dentro das malhas trianguladas.
...
O mérito principal desta versão do relatório é tornar essa leitura mais tecnicamente defensável sem perder fluidez. A parte teórica volta a ser o corpo do texto, as imagens ganham um espaço próprio de interpretação, e a rastreabilidade é consolidada em quadros finais que ligam conceitos de referência, evidência concreta e notas com páginas dos slides e linhas de código. Assim, o documento volta a se aproximar do padrão analítico da versão longa, mas evita a fragmentação causada por blocos locais de rastreabilidade a cada subseção.

Em síntese, o repositório entrega um ray tracer educacional reprodutível, com fronteiras de escopo relativamente bem definidas. O que falta está concentrado em modelagem física mais rigorosa e em estruturas globais de aceleração, e não em inconsistência do pipeline principal. Essa distinção é importante porque permite avaliar o trabalho com precisão: não como cobertura integral do arco teórico das aulas, mas como realização coerente, incremental e tecnicamente bem localizada de uma parcela significativa desse arco.
...
Raio paramétrico, visibilidade e registro de hit & \texttt{Ray}, \texttt{Hit}, \texttt{Scene.compute\_intersection()}, \texttt{Scene.trace\_ray()} & Slides 04, pp.~7--9; PBRT 4e, caps.~1.2 e 3.5; \texttt{hit.py:11, 25}; \texttt{scene.py:33, 137}. \\
...
Câmera pinhole, filme e geração de raios primários & \texttt{Camera.generate\_ray()}, \texttt{Film.get\_sample()}, \texttt{Film.render()} & Slides 04, pp.~19--39; PBRT 4e, cap.~5.2; \texttt{camera.py:6, 28}; \texttt{film.py:74, 98, 156}. \\
...
Conceito de referência & Evidência principal & Nota \\
Antialiasing por múltiplas amostras no pixel & \texttt{uniform\_samples\_2d()}, \texttt{stratified\_grid\_samples\_2d()}, \texttt{Film.get\_samples\_for\_pixel()} & Slides 05, pp.~4--8; PBRT 4e, cap.~8; \texttt{sampling.py:15, 27, 43}; \texttt{film.py:104, 156}. \\
Instanciação afim e transformação correta de normais & \texttt{Instance.intersect()} com uso de inversa e inversa transposta & Slides 05, pp.~9--13; PBRT 4e, cap.~3.5; \texttt{shape.py:253, 262}. \\
Luz de área, amostragem do emissor e penumbra & \texttt{AreaLight.sample\_radiance()}, \texttt{AreaLight.radiance()}, padrões em \texttt{sampling.py} & Slides 05, pp.~14--23; PBRT 4e, cap.~12.4; \texttt{light.py:80, 135, 195}; \texttt{sampling.py:15, 27, 43}. \\
Caixa, AABB e método de slabs & \texttt{Box.intersect()}, \texttt{AABB.intersects()} & Slides 05, pp.~24--25; PBRT 4e, cap.~3.7; \texttt{shape.py:74, 88}; \texttt{triangle\_bvh.py:50, 65}. \\
...
Reflexão especular com Fresnel-Schlick & \texttt{ReflectiveMaterial.eval()} & Slides 05, pp.~26--28; PBRT 4e, cap.~9.3; \texttt{material.py:118, 136}. \\
Refração dielétrica, TIR e Beer-Lambert & \texttt{TransparentMaterial.eval()}, \texttt{TransparentMaterial.shadow\_transmittance()} & Slides 05, pp.~29--35; PBRT 4e, caps.~9.5 e 11.2; \texttt{material.py:176, 206, 227}. \\
...
BVH local para malhas trianguladas & \texttt{TriangleBVH}, \texttt{TriangleBVHNode.intersect()}, \texttt{TriangleBVH.\_build()} & Slides 06, pp.~22--36; PBRT 4e, cap.~7.3; \texttt{triangle\_bvh.py:104, 115, 146, 160, 198}. \\
...
\clearpage
\section*{Anexo B: Diagrama de Classes}

O diagrama de classes abaixo resume a arquitetura principal do pacote \texttt{ray\_tracing\_2}. Ele complementa os quadros de referência e evidência ao mostrar, em uma única vista estrutural, as dependências entre \texttt{Render}, \texttt{Scene}, \texttt{Film}, \texttt{Camera}, \texttt{Light}, \texttt{Material}, \texttt{TriangleMesh} e \texttt{TriangleBVH}. Nesta versão do relatório, ele é mantido como apoio de leitura arquitetural, e não como substituto da análise textual.

\begin{figure*}[!ht]
\centering
\includegraphics[width=0.95\textwidth]{../ray_tracing_2_class-diagram.puml.png}
\caption{Diagrama de classes da arquitetura principal de \texttt{ray\_tracing\_2}.}
