...
- [ ] **Iluminação global difusa completa / path tracing.** O projeto permanece centrado em iluminação direta mais raios secundários determinísticos de reflexão e refração. Path tracing requereria amostrar direções aleatórias no hemisfério para cada bounce difuso e integrar por Monte Carlo, elevando o custo por pixel em uma ou mais ordens de grandeza e exigindo dezenas a centenas de amostras por pixel para convergência visível.
- [ ] **Câmera com lente fina e profundidade de campo.** A câmera implementada é estritamente pinhole. Profundidade de campo requer um modelo de lente fina que defoca raios não paralelos ao eixo, envolvendo parâmetros adicionais (`aperture`, `focus_distance`) e múltiplas amostras por pixel para simular a abertura. A base de geração de raios em `camera.py` pode ser estendida para isso, mas não estava no escopo deste projeto.
- [ ] **Estruturas de aceleração (BVH / kd-tree).** A busca por interseções ainda é linear em `Scene.compute_intersection`. Para cenas com muitos objetos, uma BVH reduziria o custo médio de $O(n)$ para $O(\log n)$ por raio. A linearidade atual é aceitável para as cenas do enunciado, que contêm poucas primitivas.
- [ ] **Calibração radiométrica estrita entre todas as fontes.** O projeto preserva a convenção do enunciado para `PointLight` (radiância constante, sem $1/r^2$), mesmo quando ela não coincide com um modelo físico completo de queda por distância. `AreaLight` já aplica $1/r^2$, gerando uma inconsistência de escala entre as duas fontes que foi mantida deliberadamente para compatibilidade com as cenas de referência do enunciado.
