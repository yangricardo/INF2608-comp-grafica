...
def render(
  ...
  scene = build_proj1_scene(ambient=(3.0, 3.0, 3.0), max_depth=2)
  build_cornell_room(scene)
  add_req1_geometry_objects(scene)

  renderer = Render()
  renderer.render(scene=scene, cam=camera, **render_options.to_render_kwargs(name='proj1_req1_geometry'))
