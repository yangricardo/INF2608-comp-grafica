...
def render(
  ...
  scene = build_proj1_scene(ambient=glm.vec3(0.03, 0.03, 0.03), max_depth=6)
  build_cornell_room(scene)
  add_rext_reflective_objects(scene)
  add_rext_reflective_refractive_lights(scene)

  renderer = Render()
  renderer.render(scene=scene, cam=camera, **render_options.to_render_kwargs(name='proj1_rext_reflective'))


def build_cli_parser() -> argparse.ArgumentParser:
  parser = build_parser(
    'Render extension requirement: reflective objects (ReflectiveMaterial with recursive ray tracing).',
    examples=[
      'python -m ray_tracing_2.proj1_rext_reflective --width 800 --height 600 --spp 1',
      'python -m ray_tracing_2.proj1_rext_reflective --width 800 --height 600 --spp 4 --sampling_mode stratified --seed 42',
    ],
  )
