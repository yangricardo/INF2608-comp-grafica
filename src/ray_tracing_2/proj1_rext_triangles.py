...
def render(
  ...
  add_rext_triangle_objects(scene, use_bvh=False)
  add_rext_triangle_lights(scene)

  renderer = Render()
  renderer.render(scene=scene, cam=camera, **render_options.to_render_kwargs(name='proj1_rext_triangles'))


def build_cli_parser() -> argparse.ArgumentParser:
  parser = build_parser(
    'Render extension requirement: triangle geometry (TriangleMesh without BVH).',
    examples=[
      'python -m ray_tracing_2.proj1_rext_triangles --width 800 --height 600 --spp 1',
      'python -m ray_tracing_2.proj1_rext_triangles --width 800 --height 600 --spp 4 --sampling_mode stratified --seed 42',
    ],
  )
