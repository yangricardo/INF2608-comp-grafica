...
def _add_original_blocks(scene) -> None:
  small_mirror = ReflectiveMaterial(
  ...
  large_glass = TransparentMaterial(ior=1.5, attenuation=glm.vec3(0.86, 0.92, 0.98))

  small_block = Box(p_min=glm.vec3(0, 0, 0), p_max=glm.vec3(1.65, 1.65, 0.30), material=small_mirror)
  small_block = Rotate(angle_deg=-18.0, x=0, y=1, z=0, shape=small_block)
  small_block = Translate(3.40, 1.2, 5.65, small_block)

  large_block = Box(p_min=glm.vec3(0, 0, 0), p_max=glm.vec3(1.65, 3.30, 1.65), material=large_glass)
...
def build_cli_parser() -> argparse.ArgumentParser:
  parser = build_parser(
    'Render final project scene v2 with alternating box materials.',
    examples=[
      'python -m ray_tracing_2.proj1_final_v2 --width 800 --height 600 --spp 1',
      'python -m ray_tracing_2.proj1_final_v2 --width 800 --height 600 --spp 4 --sampling_mode stratified --light_sampling_mode stratified --seed 42',
      'python -m ray_tracing_2.proj1_final_v2 --calibrate-only',
    ],
  )
