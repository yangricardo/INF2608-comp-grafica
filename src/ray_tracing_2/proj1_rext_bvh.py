...
from ray_tracing_2.proj1_scene_common import (
  add_rext_triangle_lights,
  add_rext_triangle_objects,
  build_cornell_room,
  build_proj1_camera,
  build_proj1_scene,
)
...
def render(
  ...
  add_rext_triangle_objects(scene, use_bvh=True)
  add_rext_triangle_lights(scene)

  renderer = Render()
  renderer.render(scene=scene, cam=camera, **render_options.to_render_kwargs(name='proj1_rext_bvh'))


def build_cli_parser() -> argparse.ArgumentParser:
  parser = build_parser(
    'Render extension requirement: triangle geometry with local BVH acceleration (TriangleMesh accelerator=bvh).',
    examples=[
      'python -m ray_tracing_2.proj1_rext_bvh --width 800 --height 600 --spp 1',
      'python -m ray_tracing_2.proj1_rext_bvh --width 800 --height 600 --spp 4 --sampling_mode stratified --seed 42',
    ],
  )
