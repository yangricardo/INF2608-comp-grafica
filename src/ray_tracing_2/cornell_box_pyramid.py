"""
Entrada principal: monta uma Cornell Box com esfera, blocos e duas pirâmides.
...
def render(width: int = 800,
  ...
  pyramid_transparent_material = TransparentMaterial(
    ior=1.5,
    attenuation=glm.vec3(0.88, 0.94, 0.98),
  )
  pyramid_reflective_material = ReflectiveMaterial(
  ...
  left_pyramid = TriangleMesh.from_vertices_faces(
    pyramid_vertices,
    pyramid_faces,
    pyramid_transparent_material,
    name='transparent_pyramid',
  )
  right_pyramid_vertices = [glm.vec3(v.x + 2.6, v.y, v.z) for v in pyramid_vertices]
  right_pyramid = TriangleMesh.from_vertices_faces(
    right_pyramid_vertices,
    pyramid_faces,
    pyramid_reflective_material,
