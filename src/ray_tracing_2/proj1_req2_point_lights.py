...
from ray_tracing_2.proj1_scene_common import add_req2_light_probe_objects, add_req2_point_lights, build_cornell_room, build_proj1_camera, build_proj1_scene
...
def render(
  ...
  scene = build_proj1_scene(ambient=(0.03, 0.03, 0.03), max_depth=2)
  build_cornell_room(scene)
  add_req2_light_probe_objects(scene)
  add_req2_point_lights(scene)

  renderer = Render()
  renderer.render(scene=scene, cam=camera, **render_options.to_render_kwargs(name='proj1_req2_point_lights'))


def build_cli_parser() -> argparse.ArgumentParser:
  parser = build_parser(
    'Render requirement-2 scene with one or more point lights in the canonical camera framing.',
    examples=[
      'python -m ray_tracing_2.proj1_req2_point_lights --width 800 --height 600 --spp 1',
      'python -m ray_tracing_2.proj1_req2_point_lights --width 800 --height 600 --spp 4 --sampling_mode stratified --seed 42',
    ],
  )
